...
function App() {
  ...
  const exportQcReport = React.useCallback(() => {
    ...
    text('vDiscovery Image Viewer', M, 30, 18, '#58A6FF', true);
    text('QC Report', M, 52, 13, '#8B949E', false);
    text(qcResults.timestamp.toLocaleString(), W - M, 30, 9, '#8B949E', false);
    pdf.setFontSize(9); textCol('#8B949E');
    pdf.text(qcResults.timestamp.toLocaleString(), W - M, 30, { align: 'right' });
    pdf.text('Generated by vDiscovery Image Viewer', W - M, 52, { align: 'right' });
    ...
    statCards.forEach((s, i) => {
      ...
      // Accent top border
      fill(s.color + '66'); pdf.rect(cx, cy, cardW, 3, 'F');
      text(s.value, cx + cardW / 2, cy + 30, 20, s.color, true);
      pdf.text(s.value, cx + cardW / 2, cy + 30, { align: 'center' });
      pdf.setFontSize(20); textCol(s.color); pdf.setFont('helvetica', 'bold');
      pdf.text(s.value, cx + cardW / 2, cy + 30, { align: 'center' });
      pdf.setFontSize(8); textCol('#8B949E'); pdf.setFont('helvetica', 'normal');
      pdf.text(s.label.toUpperCase(), cx + cardW / 2, cy + 46, { align: 'center' });
    });
    y += 2 * (cardH + 8) + 16;
