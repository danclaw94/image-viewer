const APP_VERSION = 'v1.8.6 · 2026-03-27';

// ── Pop-out detection ──────────────────────────────────────────────────────
const IS_POPOUT = new URLSearchParams(window.location.search).has('popout');
const CHANNEL   = new BroadcastChannel('vdiscovery-image-viewer');
...
// Duplicate doc IDs in OPT
function qcDuplicateDocIds(docs) {
  const seen = {}, dupes = [];
  for (const doc of docs) {
    if (seen[doc.docId]) dupes.push({ type: 'duplicate_docid', docId: doc.docId });
    seen[doc.docId] = true;
  }
  return dupes;
}

// Documents with zero pages in OPT
function qcZeroPageDocs(docs) {
  return docs.filter(d => d.pages.length === 0).map(d => ({ type: 'zero_pages', docId: d.docId }));
}

// Image format breakdown
function qcImageStats(docs, fileIndex) {
  const formats = {}, missingCount = { count: 0 };
  let totalImages = 0;
  for (const doc of docs) {
    for (const path of doc.pages) {
      totalImages++;
      const norm = normPath(path);
      const filename = norm.split('/').pop();
      if (!fileIndex[norm] && !fileIndex[filename]) { missingCount.count++; continue; }
      const ext = path.split('.').pop().toUpperCase();
      formats[ext] = (formats[ext] || 0) + 1;
    }
  }
  return { formats, totalImages, missingCount: missingCount.count };
}

// Single-page vs multi-page doc distribution
function qcPageDistribution(docs) {
  const dist = { 1: 0, '2-5': 0, '6-10': 0, '11-50': 0, '50+': 0 };
  for (const doc of docs) {
    const n = doc.pages.length;
    if (n === 1) dist[1]++;
    else if (n <= 5) dist['2-5']++;
    else if (n <= 10) dist['6-10']++;
    else if (n <= 50) dist['11-50']++;
    else dist['50+']++;
  }
  return dist;
}
...
// ── ResizableTh ───────────────────────────────────────────────────────────
// ── PopoutViewer — standalone image panel for the detached window ─────────
function PopoutViewer() {
  const [state, setState] = React.useState({ selDocId: null, selPage: 0, zoom: 'fit-page', rotation: 0, imgSrc: null, imgLoading: false });

  React.useEffect(() => {
    const handler = e => { if (e.data.type === 'nav') setState(e.data); };
  ...
  React.useEffect(() => {
    const handler = e => {
      ...
      const map = { ArrowRight: 'next_page', ArrowDown: 'next_page', ArrowLeft: 'prev_page', ArrowUp: 'prev_page' };
      if (map[e.key]) { e.preventDefault(); CHANNEL.postMessage({ type: 'cmd', cmd: map[e.key] }); }
      if (e.key === 'r' || e.key === 'R') { e.preventDefault(); CHANNEL.postMessage({ type: 'cmd', cmd: 'tag_r' }); }
      if (e.key === 'n' || e.key === 'N') { e.preventDefault(); CHANNEL.postMessage({ type: 'cmd', cmd: 'tag_nr' }); }
      if (e.key === 'u' || e.key === 'U') { e.preventDefault(); CHANNEL.postMessage({ type: 'cmd', cmd: 'tag_u' }); }
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  }, []);

  const { selDocId, selPage, zoom, rotation, imgSrc, imgLoading } = state;
  const isRotated90 = rotation === 90 || rotation === 270;

  const btn = (label, cmd, active) => (
    React.createElement('button', {
      onClick: () => CHANNEL.postMessage({ type: 'cmd', cmd }),
      style: { background: active ? P.accentGlow : 'transparent', border: '1px solid ' + (active ? P.accent : P.border), color: active ? P.accent : P.dim, borderRadius: 4, padding: '3px 8px', cursor: 'pointer', fontSize: 11 }
    }, label)
  );

  return React.createElement('div', { style: { height: '100vh', background: P.bg, display: 'flex', flexDirection: 'column', fontFamily: sans, color: P.text, overflow: 'hidden' } },
    // Toolbar
    React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 7, padding: '6px 12px', borderBottom: '1px solid ' + P.border, background: P.surface, flexShrink: 0, flexWrap: 'wrap' } },
      React.createElement('a', { href: 'https://vdiscovery.com', target: '_blank', rel: 'noopener noreferrer', style: { lineHeight: 0 } },
        React.createElement('img', { src: 'v-outline.png', alt: 'vDiscovery', style: { height: 22 } })
      ),
      React.createElement('span', { style: { fontFamily: mono, fontSize: 12, color: P.accent, fontWeight: 600 } }, selDocId || '—'),
      React.createElement('div', { style: { flex: 1 } }),
      btn('‹', 'prev_page'), btn('›', 'next_page'),
      btn('↑ Doc', 'prev_doc'), btn('↓ Doc', 'next_doc'),
      React.createElement('span', { style: { fontFamily: mono, fontSize: 10, color: P.dim } }, 'p.' + (selPage + 1)),
      btn('−', 'zoom_out'), btn('+', 'zoom_in'),
      btn('fit page', 'fit_page', zoom === 'fit-page'),
      btn('fit width', 'fit_width', zoom === 'fit-width'),
      btn('↻ Rotate', 'rotate', rotation > 0),
      btn('R', 'tag_r'), btn('NR', 'tag_nr'), btn('U', 'tag_u'),
    ),
    // Image area
    React.createElement('div', {
      style: { flex: 1, overflow: 'auto', display: 'flex', alignItems: zoom === 'fit-page' ? 'center' : 'flex-start', justifyContent: 'center', padding: zoom === 'fit-page' ? 8 : 16, background: '#080C10' }
    },
      imgLoading && React.createElement('div', { style: { color: P.dim, fontFamily: mono, fontSize: 12 } }, 'Loading…'),
      !imgLoading && !selDocId && React.createElement('div', { style: { color: P.dim, fontFamily: mono, fontSize: 13, marginTop: 80, textAlign: 'center' } }, 'Waiting for main window…\nSelect a document there to display it here.'),
      imgSrc && React.createElement('img', {
        src: imgSrc, alt: selDocId || '',
        style: {
    ...
    React.createElement('div', { style: { position: 'fixed', bottom: 8, right: 12, fontFamily: 'monospace', fontSize: 10, color: '#30363d', pointerEvents: 'none' } }, APP_VERSION)
  );
}
...
// ── VirtualGrid ───────────────────────────────────────────────────────────
// ── DraggableColList — columns dropdown with drag-to-reorder ─────────────
function DraggableColList({ orderedColKeys, hiddenCols, setHiddenCols, setColOrder, colLabel }) {
        ...
        return (
          <div key={key}
            ...
            style={{
              ...
              background: isOver ? P.accentGlow : isDragging ? P.rowHov : 'transparent',
              borderLeft: isOver ? '3px solid ' + P.accent : '3px solid transparent',
              opacity: isDragging ? 0.5 : 1,
              transition: 'background 0.08s',
            }}
            onMouseEnter={e => { if (dragIdx === null) e.currentTarget.style.background = P.rowHov; }}
            onMouseLeave={e => { if (dragIdx === null) e.currentTarget.style.background = 'transparent'; }}
          >
            {/* Drag handle */}
            <svg width="8" height="14" viewBox="0 0 8 14" fill="none" style={{ flexShrink: 0, opacity: 0.35 }}>
              <circle cx="2.5" cy="2.5" r="1.5" fill={P.dim}/>
              <circle cx="5.5" cy="2.5" r="1.5" fill={P.dim}/>
              <circle cx="2.5" cy="7" r="1.5" fill={P.dim}/>
              <circle cx="5.5" cy="7" r="1.5" fill={P.dim}/>
              <circle cx="2.5" cy="11.5" r="1.5" fill={P.dim}/>
              <circle cx="5.5" cy="11.5" r="1.5" fill={P.dim}/>
            </svg>
            {/* Checkbox */}
            <div onClick={e => { e.stopPropagation(); setHiddenCols(prev => { const n = new Set(prev); vis ? n.add(key) : n.delete(key); return n; }); }}
              style={{ width: 15, height: 15, borderRadius: 3, border: '2px solid ' + (vis ? P.accent : P.border), background: vis ? P.accent : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.1s', cursor: 'pointer' }}>
            ...
            <span style={{ fontFamily: "'JetBrains Mono','Fira Code','SF Mono',monospace", fontSize: 11, color: vis ? P.text : P.dim, flex: 1 }}>{colLabel(key)}</span>
          </div>
        );
      })}
    </div>
...
function VirtualGrid({ rows, headers, selDocId, selectedDocIds, onSelect, onMultiSelect, tagMap, hiddenCols, colWidths, sortCol, sortDir, onSort, setColWidths, orderedColKeys, colLabel }) {
      ...
      <table style={{ borderCollapse: 'collapse', fontFamily: mono, fontSize: 12, whiteSpace: 'nowrap', tableLayout: 'fixed' }}>
        ...
        <thead style={{ position: 'sticky', top: 0, zIndex: 2 }}>
          <tr style={{ background: P.surface, borderBottom: '2px solid ' + P.border }}>
            ...
            {(orderedColKeys || ['tag','docid','pages']).map(key => {
              if (hidden(key)) return null;
              if (key === 'tag')   return <ResizableTh key="tag" colKey="tag" width={cw('tag')} onResize={w => setColWidths(p => ({ ...p, tag: w }))} onSort={() => onSort('tag')} active={sortCol === 'tag'} sortDir={sortDir}>Tag</ResizableTh>;
              if (key === 'docid') return <ResizableTh key="docid" colKey="docid" width={cw('docid')} onResize={w => setColWidths(p => ({ ...p, docid: w }))} onSort={() => onSort(-1)} active={sortCol === -1} sortDir={sortDir}>DOCID</ResizableTh>;
              if (key === 'pages') return <ResizableTh key="pages" colKey="pages" width={cw('pages')} onResize={w => setColWidths(p => ({ ...p, pages: w }))} onSort={() => onSort('pages')} active={sortCol === 'pages'} sortDir={sortDir} align="center">Pages</ResizableTh>;
              const i = parseInt(key.slice(3));
              const h = headers[i + 1];
              if (!h) return null;
              return <ResizableTh key={key} colKey={key} width={cw(key)} onResize={w => setColWidths(p => ({ ...p, [key]: w }))} onSort={() => onSort(i + 1)} active={sortCol === (i + 1)} sortDir={sortDir}>{h}</ResizableTh>;
            })}
          </tr>
        </thead>
            ...
            return (
              <tr key={row.docId}
                ...
                {(orderedColKeys || ['tag','docid','pages']).map(key => {
                  if (hidden(key)) return null;
                  if (key === 'tag') return (
                    <td key="tag" style={{ width: cw('tag'), padding: '0 4px', borderRight: '1px solid ' + P.border, textAlign: 'center', overflow: 'hidden' }}>
                  ...
                  if (key === 'docid') return <td key="docid" style={{ width: cw('docid'), padding: '0 10px', borderRight: '1px solid ' + P.border, color: isSel ? P.accent : P.text, fontWeight: isSel ? 600 : 400, overflow: 'hidden', textOverflow: 'ellipsis' }}>{row.docId}</td>;
                  if (key === 'pages') return <td key="pages" style={{ width: cw('pages'), padding: '0 8px', borderRight: '1px solid ' + P.border, color: P.dim, textAlign: 'center', fontSize: 11 }}>{row.pages.length}</td>;
                  const ci = parseInt(key.slice(3));
                  return <td key={key} style={{ width: cw(key), padding: '0 10px', borderRight: '1px solid ' + P.border, color: P.dim, overflow: 'hidden', textOverflow: 'ellipsis' }}>{row.fields[ci + 1] || ''}</td>;
                })}
                <td style={{ width: 28 }} />
              </tr>
            );
...
function App() {
  ...
  const [colOrder,     setColOrder]     = React.useState(null); // null = default order; array of keys when reordered
  ...
  const popoutWinRef  = React.useRef(null);
  const [isPopped,  setIsPopped]  = React.useState(false); // main window has popped out image
  ...
  }, [tagMap, tagFilter, selDocId, search, hiddenCols, colWidths, colOrder, launched, optFile]);
  ...
  const runQcScan = React.useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      const duplicateIds   = qcDuplicateDocIds(docs);
      const zeroPageDocs   = qcZeroPageDocs(docs);
      const imageStats     = qcImageStats(docs, fileIndex);
      const pageDist       = qcPageDistribution(docs);
      const totalImages    = docs.reduce((s, d) => s + d.pages.length, 0);
      const totalIssues    = missingFiles.length + datMismatches.length + batesGaps.reduce((s,g)=>s+g.count,0) + duplicateIds.length + zeroPageDocs.length;
      setQcResults({ missingFiles, datMismatches, batesGaps, duplicateIds, zeroPageDocs, imageStats, pageDist, totalImages, totalIssues, timestamp: new Date() });
  ...
  // ── Broadcast navigation to pop-out window ──
  React.useEffect(() => {
    if (!isPopped) return;
    CHANNEL.postMessage({ type: 'nav', selDocId, selPage, zoom, rotation, imgSrc, imgLoading });
  }, [selDocId, selPage, zoom, rotation, imgSrc, imgLoading, isPopped]);

  // ── Receive nav commands from pop-out (arrow keys driven there) ──
  React.useEffect(() => {
    const handler = e => {
      if (e.data.type === 'cmd') {
        const { cmd } = e.data;
        if (cmd === 'next_page')  setSelPage(p => Math.min((selRow ? selRow.pages.length - 1 : 0), p + 1));
        if (cmd === 'prev_page')  setSelPage(p => Math.max(0, p - 1));
        if (cmd === 'next_doc') {
          const idx = filteredRows.findIndex(r => r.docId === selDocId);
          if (idx >= 0 && idx < filteredRows.length - 1) { setSelDocId(filteredRows[idx + 1].docId); setSelPage(0); }
        }
        if (cmd === 'prev_doc') {
          const idx = filteredRows.findIndex(r => r.docId === selDocId);
          if (idx > 0) { setSelDocId(filteredRows[idx - 1].docId); setSelPage(0); }
        }
        if (cmd === 'zoom_in')  setZoom(z => Math.min(4, (typeof z === 'number' ? z : 1) + 0.25));
        if (cmd === 'zoom_out') setZoom(z => Math.max(0.25, (typeof z === 'number' ? z : 1) - 0.25));
        if (cmd === 'fit_page') setZoom('fit-page');
        if (cmd === 'fit_width')setZoom('fit-width');
        if (cmd === 'rotate')   setRotation(r => (r + 90) % 360);
        if (cmd === 'tag_r')  setTag(selDocId, tagMap.get(selDocId) === 'responsive' ? null : 'responsive');
        if (cmd === 'tag_nr') setTag(selDocId, tagMap.get(selDocId) === 'not_responsive' ? null : 'not_responsive');
        if (cmd === 'tag_u')  setTag(selDocId, null);
      }
    };
    CHANNEL.addEventListener('message', handler);
    return () => CHANNEL.removeEventListener('message', handler);
  }, [selDocId, selRow, filteredRows, tagMap, setTag]);

  // ── Heartbeat — detect when pop-out closes ──
  React.useEffect(() => {
    if (!isPopped) return;
    const t = setInterval(() => {
      if (popoutWinRef.current && popoutWinRef.current.closed) {
        ...
        setIsPopped(false);
        showNotice('ok', 'Image viewer returned to main window', 3000);
      }
    }, 800);
  ...
  const openPopout = React.useCallback(() => {
    const url = window.location.href.split('?')[0] + '?popout=1';
    const w = window.open(url, 'vd-image-viewer', 'width=900,height=700,menubar=no,toolbar=no,location=no,status=no');
    if (w) {
      popoutWinRef.current = w;
      setIsPopped(true);
      // Send initial state after pop-out loads
      setTimeout(() => {
        CHANNEL.postMessage({ type: 'nav', selDocId, selPage, zoom, rotation, imgSrc, imgLoading });
      }, 800);
    }
  }, [selDocId, selPage, zoom, rotation, imgSrc, imgLoading]);
  ...
  const exportQcReport = React.useCallback(() => {
    ...
    const checkPage = (need = 20) => { if (y + need > H - M) { pdf.addPage(); y = M; } };

    // Colour helpers
    const rgb = hex => hexToRgb(hex);
    const fill = hex => pdf.setFillColor(...rgb(hex));
    const stroke = hex => pdf.setDrawColor(...rgb(hex));
    const textCol = hex => pdf.setTextColor(...rgb(hex));

    const text = (t, x, yy, size, hex, bold, maxW) => {
      pdf.setFontSize(size);
      textCol(hex);
      pdf.setFont('helvetica', bold ? 'bold' : 'normal');
      if (maxW) {
        const lines = pdf.splitTextToSize(String(t), maxW);
        pdf.text(lines, x, yy);
        return lines.length * size * 1.3;
      }
      pdf.text(String(t), x, yy);
      return size * 1.3;
    };
    ...
    pdf.roundedRect(M, y, CW, 36, 4, 4, 'FD');
    text('OPT:', M + 10, y + 14, 8, '#8B949E', true);
    text(optFile || 'N/A', M + 30, y + 14, 8, '#E6EDF3', false, CW / 2 - 40);
    text('DAT:', M + CW / 2 + 5, y + 14, 8, '#8B949E', true);
    text(datFile || 'N/A', M + CW / 2 + 25, y + 14, 8, '#E6EDF3', false, CW / 2 - 30);
    text('Image Folder:', M + 10, y + 28, 8, '#8B949E', true);
    text(imgDir || 'N/A', M + 75, y + 28, 8, '#E6EDF3', false, CW - 80);
    y += 48;

    // ── Summary stat cards (2 rows × 3 cols) ──────────────────────────────
    const totalDocs  = gridRows.length;
    const totalImages = qcResults.totalImages;
    const totalPages1 = docs.reduce((s, d) => s + d.pages.length, 0);
    const avgPages   = totalDocs > 0 ? (totalPages1 / totalDocs).toFixed(1) : '0';
    const pctMissing = totalImages > 0 ? ((qcResults.missingFiles.length / totalImages) * 100).toFixed(1) : '0';
    const maxPages   = docs.reduce((mx, d) => Math.max(mx, d.pages.length), 0);

    const statCards = [
      { label: 'Documents', value: totalDocs.toLocaleString(), color: '#58A6FF' },
      { label: 'Total Images', value: totalImages.toLocaleString(), color: '#58A6FF' },
      { label: 'Avg Pages/Doc', value: avgPages, color: '#8B949E' },
      { label: 'Max Pages/Doc', value: maxPages.toLocaleString(), color: '#8B949E' },
      { label: 'Missing Images', value: qcResults.missingFiles.length.toLocaleString(), color: qcResults.missingFiles.length > 0 ? '#F85149' : '#3FB950' },
      { label: 'QC Issues', value: qcResults.totalIssues.toLocaleString(), color: qcResults.totalIssues > 0 ? '#D29922' : '#3FB950' },
    ];
    const cardW = (CW - 16) / 3, cardH = 54;
    statCards.forEach((s, i) => {
      const col = i % 3, row = Math.floor(i / 3);
      const cx = M + col * (cardW + 8), cy = y + row * (cardH + 8);
      fill('#161B22'); stroke('#30363D');
      pdf.roundedRect(cx, cy, cardW, cardH, 4, 4, 'FD');
      // Accent top border
      fill(s.color); pdf.rect(cx, cy, cardW, 3, 'F');
      pdf.setFontSize(22); textCol(s.color); pdf.setFont('helvetica', 'bold');
      pdf.text(s.value, cx + cardW / 2, cy + 32, { align: 'center' });
      pdf.setFontSize(8); textCol('#8B949E'); pdf.setFont('helvetica', 'normal');
      pdf.text(s.label.toUpperCase(), cx + cardW / 2, cy + 47, { align: 'center' });
    });
    y += 2 * (cardH + 8) + 16;
    ...
    if (formatEntries.length > 0) {
      checkPage(80);
      text('Image Format Breakdown', M, y, 11, '#E6EDF3', true); y += 16;
      const maxCount = Math.max(...formatEntries.map(e => e[1]));
      const barMaxW = CW - 120;
      const barColors = ['#58A6FF', '#3FB950', '#D29922', '#F85149', '#a78bfa', '#8B949E'];
      formatEntries.forEach(([fmt, count], i) => {
        checkPage(22);
        const barW = maxCount > 0 ? (count / maxCount) * barMaxW : 0;
        const pct = totalImages > 0 ? ((count / totalImages) * 100).toFixed(1) : '0';
        const col = barColors[i % barColors.length];
        pdf.setFontSize(9); textCol('#8B949E'); pdf.setFont('helvetica', 'normal');
        pdf.text(fmt, M, y + 10);
        fill(col + '33'); pdf.rect(M + 40, y, barW, 14, 'F');
        fill(col); pdf.rect(M + 40, y, barW, 14, 'F');
        pdf.setFontSize(8); textCol('#E6EDF3');
        pdf.text(count.toLocaleString() + '  (' + pct + '%)', M + 40 + barW + 6, y + 10);
        y += 20;
      });
      y += 8;
    }
    ...
    // ── Page Distribution ──────────────────────────────────────────────────
    checkPage(80);
    const dist = qcResults.pageDist;
    const distEntries = [['1 page', dist[1]], ['2–5 pages', dist['2-5']], ['6–10 pages', dist['6-10']], ['11–50 pages', dist['11-50']], ['50+ pages', dist['50+']]];
    if (totalDocs > 0) {
      text('Page Distribution', M, y, 11, '#E6EDF3', true); y += 16;
      const barMaxW2 = CW - 130;
      const maxD = Math.max(...distEntries.map(e => e[1]));
      distEntries.forEach(([label, count]) => {
        if (count === 0) return;
        checkPage(22);
        const barW = maxD > 0 ? (count / maxD) * barMaxW2 : 0;
        const pct = totalDocs > 0 ? ((count / totalDocs) * 100).toFixed(1) : '0';
        pdf.setFontSize(9); textCol('#8B949E'); pdf.setFont('helvetica', 'normal');
        pdf.text(label, M, y + 10);
        fill('#58A6FF33'); pdf.rect(M + 70, y, barW, 14, 'F');
        fill('#58A6FF'); pdf.rect(M + 70, y, barW, 14, 'F');
        pdf.setFontSize(8); textCol('#E6EDF3');
        pdf.text(count.toLocaleString() + '  (' + pct + '%)', M + 70 + barW + 6, y + 10);
        y += 20;
      });
      y += 8;
    }
    ...
    if (headers.length > 0) {
      checkPage(40);
      text('DAT Metadata Columns (' + headers.length + ')', M, y, 11, '#E6EDF3', true); y += 14;
      const colsPerRow = 3, colW2 = CW / colsPerRow;
      headers.forEach((h, i) => {
        checkPage(16);
        const col = i % colsPerRow, row2 = Math.floor(i / colsPerRow);
        if (col === 0 && i > 0) y += 16;
        const cx = M + col * colW2;
        // Count non-empty values
        const filled = allRows.filter(r => r.fields[i] && r.fields[i].trim()).length;
        const pct = allRows.length > 0 ? Math.round((filled / allRows.length) * 100) : 0;
        const dotCol = pct > 80 ? '#3FB950' : pct > 40 ? '#D29922' : '#F85149';
        fill(dotCol); pdf.circle(cx + 5, y + 5, 3, 'F');
        pdf.setFontSize(8); textCol('#E6EDF3'); pdf.setFont('helvetica', 'bold');
        pdf.text(h, cx + 12, y + 8);
        textCol('#8B949E'); pdf.setFont('helvetica', 'normal');
        pdf.text(pct + '% filled', cx + colW2 - 10, y + 8, { align: 'right' });
      });
      y += 24;
    }

    // ── Issues Detail ──────────────────────────────────────────────────────
    const issueSection = (title, color, items, renderItem) => {
      if (!items.length) return;
      checkPage(30);
      // Section header band
      fill(color + '22'); pdf.rect(M, y, CW, 18, 'F');
      fill(color); pdf.rect(M, y, 3, 18, 'F');
      pdf.setFontSize(10); textCol(color); pdf.setFont('helvetica', 'bold');
      pdf.text(title, M + 10, y + 12);
      pdf.setFontSize(9); textCol('#8B949E'); pdf.setFont('helvetica', 'normal');
      pdf.text(items.length.toLocaleString() + ' items', W - M, y + 12, { align: 'right' });
      y += 24;
      const MAX_SHOWN = 50;
      ...
      if (items.length > MAX_SHOWN) {
        pdf.setFontSize(8); textCol('#8B949E');
        pdf.text('… and ' + (items.length - MAX_SHOWN) + ' more (see CSV export for full list)', M + 4, y);
        y += 14;
      }
      y += 6;
    };

    issueSection('Missing Image Files', '#F85149', qcResults.missingFiles, i => {
      pdf.setFontSize(8); textCol('#F85149'); pdf.setFont('helvetica', 'bold');
      pdf.text('✗', M + 4, y + 10);
      textCol('#E6EDF3'); pdf.setFont('helvetica', 'normal');
      pdf.text(i.docId + ' — p.' + (i.page + 1), M + 14, y + 10);
      textCol('#8B949E');
      pdf.text(i.path, M + 14, y + 10, { maxWidth: CW - 18 });
      const lines = pdf.splitTextToSize(i.path, CW - 100);
      pdf.text(i.docId + ' — p.' + (i.page + 1) + '  ', M + 14, y + 10);
      textCol('#8B949E'); pdf.setFontSize(7);
      pdf.text(i.path, M + 14 + pdf.getTextWidth(i.docId + ' — p.' + (i.page + 1) + '  '), y + 10);
    });

    issueSection('OPT / DAT Mismatches', '#D29922', qcResults.datMismatches, i => {
      pdf.setFontSize(8); textCol('#D29922'); pdf.setFont('helvetica', 'bold');
      pdf.text('!', M + 6, y + 10);
      textCol('#E6EDF3'); pdf.setFont('helvetica', 'normal');
      pdf.text(i.docId, M + 14, y + 10);
      textCol('#8B949E');
      pdf.text(i.type === 'opt_only' ? 'In OPT only — no matching DAT record' : 'In DAT only — no matching OPT entry', M + 14 + pdf.getTextWidth(i.docId) + 6, y + 10);
    });

    issueSection('Bates Sequence Gaps', '#D29922', qcResults.batesGaps, g => {
      pdf.setFontSize(8); textCol('#D29922'); pdf.setFont('helvetica', 'bold');
      pdf.text('GAP', M + 2, y + 10);
      textCol('#E6EDF3'); pdf.setFont('helvetica', 'normal');
      const range = g.from + (g.count > 1 ? ' – ' + g.to : '');
      pdf.text(range, M + 24, y + 10);
      textCol('#8B949E');
      pdf.text(g.count + ' missing  (after ' + g.afterDocId + ')', M + 24 + pdf.getTextWidth(range) + 6, y + 10);
    });

    issueSection('Duplicate Document IDs', '#F85149', qcResults.duplicateIds || [], i => {
      pdf.setFontSize(8); textCol('#F85149'); pdf.setFont('helvetica', 'bold');
      pdf.text('DUP', M + 2, y + 10);
      textCol('#E6EDF3'); pdf.setFont('helvetica', 'normal');
      pdf.text(i.docId, M + 24, y + 10);
    });

    issueSection('Zero-Page Documents', '#F85149', qcResults.zeroPageDocs || [], i => {
      pdf.setFontSize(8); textCol('#F85149'); pdf.setFont('helvetica', 'bold');
      pdf.text('0p', M + 2, y + 10);
      textCol('#E6EDF3'); pdf.setFont('helvetica', 'normal');
      pdf.text(i.docId, M + 24, y + 10);
    });
  ...
  const exportQcCsv = React.useCallback(() => {
    ...
    for (const i of (qcResults.duplicateIds || []))
      rows.push(['Duplicate Doc ID', i.docId, 'Appears more than once in OPT']);
    for (const i of (qcResults.zeroPageDocs || []))
      rows.push(['Zero Pages', i.docId, 'Document has no image pages']);
  ...
  const GRID_W     = (selDocId && !isPopped) ? '55%' : '100%';
  const IMG_W      = (selDocId && !isPopped) ? '45%' : '0';
  const isRotated90 = rotation === 90 || rotation === 270;
        ...
        <div style={{ flexShrink: 0 }}>
            ...
            {qcRunning ? (
              ...
              <>
                ...
                <span>QC: {qcResults.totalIssues > 0
                  ? qcResults.totalIssues + ' issues — ' + [qcResults.missingFiles.length > 0 && qcResults.missingFiles.length + ' missing files', qcResults.datMismatches.length > 0 && qcResults.datMismatches.length + ' OPT/DAT mismatches', qcResults.batesGaps.length > 0 && qcResults.batesGaps.reduce((s,g)=>s+g.count,0) + ' Bates gaps', (qcResults.duplicateIds||[]).length > 0 && (qcResults.duplicateIds||[]).length + ' duplicate IDs', (qcResults.zeroPageDocs||[]).length > 0 && (qcResults.zeroPageDocs||[]).length + ' zero-page docs'].filter(Boolean).join(', ')
                  : 'No issues found'
                }</span>
          ...
          {!qcRunning && qcExpanded && qcResults.totalIssues > 0 && (
            <div style={{ maxHeight: 180, overflowY: 'auto', borderBottom: '1px solid ' + P.border, background: P.surface, padding: '6px 16px', fontFamily: mono, fontSize: 11 }}>
              ...
              {(qcResults.duplicateIds || []).map((i, idx) => (
                <div key={idx} onClick={() => selectDoc(i.docId)} style={{ padding: '2px 0', cursor: 'pointer', color: P.red }}>
                  ✗ Duplicate ID: {i.docId}
                </div>
              ))}
              {(qcResults.zeroPageDocs || []).map((i, idx) => (
                <div key={idx} onClick={() => selectDoc(i.docId)} style={{ padding: '2px 0', cursor: 'pointer', color: P.red }}>
                  ✗ Zero pages: {i.docId}
                </div>
              ))}
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ width: GRID_W, transition: 'width 0.12s', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: selDocId ? '1px solid ' + P.border : 'none' }}>
          ...
          <VirtualGrid
            ...
            setColWidths={setColWidths} orderedColKeys={orderedColKeys} colLabel={colLabel}
          />
        </div>
          ...
          <div style={{ width: IMG_W, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '4px 10px', borderBottom: '1px solid ' + P.border, flexShrink: 0, background: P.bg, flexWrap: 'wrap' }}>
              ...
              <div style={{ position: 'relative' }}>
                {/* Pop-out button */}
                {!isPopped
                  ? <button onClick={openPopout} title="Pop out image to separate window" style={{ background: 'transparent', border: '1px solid ' + P.border, color: P.dim, borderRadius: 4, padding: '2px 7px', cursor: 'pointer', fontSize: 11 }}>⧉</button>
                  : <button onClick={() => { if (popoutWinRef.current && !popoutWinRef.current.closed) popoutWinRef.current.close(); setIsPopped(false); }} title="Close pop-out and return image here" style={{ background: P.accentGlow, border: '1px solid ' + P.accent, color: P.accent, borderRadius: 4, padding: '2px 7px', cursor: 'pointer', fontSize: 11 }}>⧉ ✕</button>
                }
...
root.render(
  IS_POPOUT
    ? React.createElement(PopoutViewer, null)
    : React.createElement(React.Fragment, null, React.createElement(App, null), React.createElement(VersionBadge, null))
);
